...
import { and as and2, assign as assign2, enqueueActions as enqueueActions3, log as log2, not as not2, or, raise as raise2, sendTo as sendTo3, setup as setup4 } from "xstate";

// src/internals/machines/sign-up/start.machine.ts
import { assertEvent, enqueueActions, fromPromise as fromPromise2, not, sendTo, setup as setup2 } from "xstate";
...
var SignUpStartMachine = setup2({
  actors: {
    attempt: fromPromise2(({ input: { fields, parent, params } }) => {
...
import { and, assign, enqueueActions as enqueueActions2, fromCallback, fromPromise as fromPromise3, log, raise, sendParent, sendTo as sendTo2, setup as setup3 } from "xstate";
...
var SignUpVerificationMachine = setup3({
  actors: {
    prepare: fromPromise3(({ input: { params, parent } }) => {
...
var SignUpRouterMachine = setup4({
  ...
  guards: {
    ...
    hasTicket: ({ context }) => Boolean(context.ticket),
    isLoggedInAndSingleSession: and2(["isLoggedIn", "isSingleSessionMode", not2("isExampleMode")]),
  ...
  on: {
    ...
    "AUTHENTICATE.SAML": {
      actions: sendTo3(ThirdPartyMachineId, ({ context }) => {
        var _a, _b, _c, _d, _e;
        return {
          type: "REDIRECT",
          params: {
    ...
    "AUTHENTICATE.WEB3": {
      actions: sendTo3("start", ({ event }) => event)
    },
    "FORM.ATTACH": {
      description: "Attach/re-attach the form to the router.",
      actions: enqueueActions3(({ enqueue, event }) => {
        enqueue.assign({
  ...
  states: {
    ...
    Init: {
      entry: enqueueActions3(({ context, enqueue, self }) => {
        var _a, _b;
    ...
    Start: {
      ...
      on: {
        ...
        NEXT: [
          ...
          {
            guard: and2(["hasTicket", "statusNeedsContinue"]),
...
function SignUpFlowProvider({ children, exampleMode, fallback, isRootPath }) {
  ...
  return /* @__PURE__ */ jsxs(SignUpRouterCtx.Provider, { actorRef: actor, children: [
